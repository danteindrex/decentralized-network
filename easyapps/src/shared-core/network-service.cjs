...
// Configuration that gets updated by the auto-discovery system
let config = {
    eth_node: 'https://bootstrap-node.onrender.com',
    ipfs_host: 'bootstrap-node.onrender.com',
    ipfs_port: 443,
    contract_address: '', // Will be discovered from bootstrap
    model_registry_address: '', // Will be discovered from bootstrap
    default_account: '', // Will be generated automatically
    private_key: '' // Will be generated automatically
};
...
function initWeb3() {
    try {
        ...
        const w3 = new Web3(config.eth_node);
...
// Chat message storage functions - now with user address tracking
async function saveChatMessageToIpfs(message, userAddress = null) {
    try {
        // Enhance message with user metadata
        const enhancedMessage = {
            ...message,
            userAddress: userAddress || config.default_account,
            timestamp: message.timestamp || new Date().toISOString(),
            networkId: config.network_id || 'tensor-parallelism-network'
        };
        
        const fileName = `chat_${userAddress || config.default_account}_${Date.now()}.json`;
        const cid = await uploadToIpfs(JSON.stringify(enhancedMessage), fileName, true);
        
        if (cid) {
            await addChatCid(cid, enhancedMessage, userAddress);
            return cid;
        }
        return null;
    } catch (error) {
...
async function loadChatMessagesFromIpfs(userAddress = null) {
    try {
        const cidEntries = await getChatCids(userAddress || config.default_account);
        const messages = [];
        
        for (const cidEntry of cidEntries) {
            const cid = typeof cidEntry === 'string' ? cidEntry : cidEntry.cid;
            const message = await fetchFromIpfs(cid);
            if (message) {
                // Add CID metadata to message
                message.cidInfo = typeof cidEntry === 'object' ? cidEntry : { cid };
...
// Persistent storage for chat CIDs - now per user address
function getChatCidsFilePath(userAddress = null) {
    if (!userDataPath) {
        console.error("User data path not set for chat CIDs.");
        return null;
    }
    
    // Use user address for unique file naming
    const fileName = userAddress ? `chat_cids_${userAddress.toLowerCase()}.json` : CHAT_CIDS_FILE;
    return path.join(userDataPath, fileName);
}

async function getChatCids(userAddress = null) {
    const filePath = getChatCidsFilePath(userAddress || config.default_account);
    if (!filePath) return [];

    try {
        if (fs.existsSync(filePath)) {
            const data = fs.readFileSync(filePath, 'utf8');
            const cidData = JSON.parse(data);
            // Return array of CID objects with metadata
            return Array.isArray(cidData) ? cidData : (cidData.cids || []);
        }
        return [];
    } catch (error) {
...
async function addChatCid(cid, messageData = {}, userAddress = null) {
    const filePath = getChatCidsFilePath(userAddress || config.default_account);
    if (!filePath) return false;

    try {
        const cids = await getChatCids(userAddress || config.default_account);
        
        // Store CID with metadata
        const cidEntry = {
            cid,
            timestamp: new Date().toISOString(),
            userAddress: userAddress || config.default_account,
            messageType: messageData.role || 'user',
            prompt: messageData.content ? messageData.content.substring(0, 100) + '...' : '',
            jobId: messageData.jobId || null,
            txHash: messageData.txHash || null
        };
        ...
        const fileData = {
            userAddress: userAddress || config.default_account,
            createdAt: cids.length === 1 ? new Date().toISOString() : undefined,
            lastUpdated: new Date().toISOString(),
            totalMessages: cids.length,
            cids
        };
        
        fs.writeFileSync(filePath, JSON.stringify(fileData, null, 2));
        console.log(`Chat CID added for user ${userAddress || config.default_account}:`, cid);
        return true;
    } catch (error) {
...
async function generateAutoConfig(bootstrapNode = null) {
    ...
    // Default bootstrap nodes (updated with Render deployment)
    const defaultBootstrapNodes = [
        { url: 'https://bootstrap-node.onrender.com', ipfs: 'bootstrap-node.onrender.com:443' },
    ...
    for (const node of bootstrapNodes) {
        try {
            ...
            // For HTTPS bootstrap nodes, use the same URL, for local nodes replace port
            const configUrl = node.url.startsWith('https://') 
                ? `${node.url}/api/network-config`
                : `${node.url.replace(':8545', ':8080')}/api/network-config`;
            ...
            if (response.status === 200 && response.data) {
                ...
                // Use the first bootstrap node URL from the response, or fallback to RPC proxy
                const ethNodeUrl = response.data.bootstrap_nodes && response.data.bootstrap_nodes.length > 0 
                    ? response.data.bootstrap_nodes[0].url 
                    : `${node.url}/rpc`;
    ...
    // Fallback configuration using discovered bootstrap nodes
    console.log('Using fallback configuration with discovered bootstrap nodes');
    const primaryBootstrap = bootstrapNodes[0]; // Use first available bootstrap
    return {
        eth_node: primaryBootstrap.url,
        ipfs_host: primaryBootstrap.ipfs.split(':')[0],
        ipfs_port: parseInt(primaryBootstrap.ipfs.split(':')[1] || 5001),
        contract_address: '',
...
async function getUserChatStats(userAddress = null) {
    try {
        const address = userAddress || config.default_account;
        const cidEntries = await getChatCids(address);
        
        return {
            userAddress: address,
            totalMessages: cidEntries.length,
            firstMessageDate: cidEntries.length > 0 ? cidEntries[0].timestamp : null,
            lastMessageDate: cidEntries.length > 0 ? cidEntries[cidEntries.length - 1].timestamp : null,
            totalInferenceJobs: cidEntries.filter(entry => entry.jobId).length,
            storageUsed: cidEntries.length * 1024 // Approximate storage per message
        };
    } catch (error) {
